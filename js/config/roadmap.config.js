...
export const STAGES = [
  ...
  },
  {
    ...
    actionRequired: true,
    severity: "active",
    // 22.09.2026: было "✅" — на плашке ТЕКУЩЕГО этапа (roadmap.js показывает
    // иконку конфига только для current, завершённые рисуются галочкой)
    // зелёный чекбокс читался как «выполнено». Этап — «ищите офер», не финал.
    icon: "🧭",
    ...
    // 2026-09-11: кнопка "У меня есть Job Offer" — уведомляет координатора
    // через amoCRM-задачу; координатор вручную связывается со студентом и
    // переводит сделку на этап "Job Offer получен" (JOB_OFFER_SENT_INTL_REP),
    // где уже настроена видео-инструкция по загрузке офера.
    cta: { label: "У меня есть Job Offer 🎉", action: "confirmJobOffer" },
  },

  // ----------------------------------------------------------------- JOB OFFER
  // ЭТАП ЗАВЕДЁН 14.09.2026. В воронке amoCRM он называется «Обратная связь»
  // и стоит сразу после «Заполнен личный кабинет». До этого дня приложение о
  // нём не знало: сделка переезжала в него, а студент продолжал видеть
  // предыдущий этап и не получал уведомления.
  {
    id: "JOB_SEARCH",
    group: "job_offer",
    order: 4.5,
    title: "Ищем работодателя",
    shortTitle: "Поиск работы",
    actionRequired: true,
    severity: "active",
    icon: "🔎",
    description: "Вы ищете работодателя, мы на связи и помогаем. Как только офер будет на руках — нажмите кнопку ниже.",
    detail: {
      whatsHappening: "Идёт поиск работодателя. Координатор на связи и подсказывает по вакансиям и переписке.",
      whatRequired: "Искать работодателя и держать координатора в курсе.",
      whatsNext: "Когда офер получен, его нужно будет загрузить в личный кабинет CIEE.",
    },
    // Та же кнопка, что и на предыдущем этапе: студент может найти офер в
    // любой момент этого периода, и сообщить об этом он должен одинаково.
    cta: { label: "У меня есть Job Offer 🎉", action: "confirmJobOffer" },
  },
  {
  ...
  // ЭТАП ЗАВЕДЁН 14.09.2026. В воронке — «Подать офер на проверку». Это
  // единственный шаг блока Job Offer, где действие за студентом: офер
  // загружает он сам, в свой личный кабинет CIEE. Раньше шаг был невидимым,
  // и просьба загрузить офер доходила только личным сообщением координатора.
  {
    id: "JOB_OFFER_UPLOAD_CIEE",
    group: "job_offer",
    order: 5.3,
    title: "Загрузите офер в кабинет CIEE",
    shortTitle: "Загрузить офер",
    actionRequired: true,
    severity: "danger",
    icon: "📤",
    description: "Зайдите в личный кабинет CIEE и загрузите Job Offer на проверку.",
    detail: {
      whatsHappening: "Офер проверен ABC Universe, его нужно подать спонсору через ваш личный кабинет.",
      whatRequired: "Зайти в личный кабинет CIEE и загрузить офер. Инструкция — по кнопке ниже.",
      whatsNext: "Мы передадим офер спонсору CIEE и сообщим, когда начнётся проверка.",
    },
    cta: { label: "Открыть инструкцию", action: "openInstruction" },
    instructionUrl: "https://drive.google.com/drive/folders/1A-3U7-rCT2U51FblQINQmtvOlKfcaI-6",
  },
  // ЭТАП ЗАВЕДЁН 14.09.2026. В воронке статус называется «Job Submitted to
  // CIEE Oxana» — имя сотрудника в названии внутреннее, студенту его видеть
  // незачем, поэтому здесь шаг назван по сути. Он отличается от следующего:
  // здесь мы офер ОТПРАВИЛИ, а на «Передан в CIEE» спонсор начал проверку.
  {
    id: "JOB_OFFER_SENT_CIEE",
    group: "job_offer",
    order: 5.6,
    title: "Офер отправлен в CIEE",
    shortTitle: "Отправлен в CIEE",
    actionRequired: false,
    severity: "active",
    icon: "📨",
    description: "Мы передали ваш офер спонсору CIEE. От вас ничего не требуется.",
    detail: {
      whatsHappening: "Job Offer отправлен спонсору CIEE и поставлен в очередь на проверку.",
      whatRequired: "Пока ничего.",
      whatsNext: "Спонсор начнёт проверку офера и может запросить документы у работодателя.",
    },
    cta: null,
  },
  {
